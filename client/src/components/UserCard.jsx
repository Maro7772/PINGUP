...
const UserCard = ({ user }) => {
  ...
  return (
    <div
      key={user._id}
      className="p-4 pt-6 flex flex-col justify-between w-full sm:w-72 max-w-full shadow border border-gray-200 rounded-md"
    >
      ...
      <div className="flex items-center justify-center gap-2 mt-4 text-xs text-gray-600">
        ...
        <div className="flex items-center gap-1 border border-gray-300 rounded-full px-3 py-1">
          <MapPin className="w-4 h-4" /> <span>{user.followers.length}</span>
          Followers
        </div>
      </div>
